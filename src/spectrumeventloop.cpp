...
static gboolean processEvent(void *data) {
	Event *ev = (Event *) data;
	ev->callback();
	Log("SpectrumEventLoop", "Event " << ev << " callback called");
	delete ev;
	return FALSE;
}
...
void SpectrumEventLoop::run() {
	Log("SpectrumEventLoop", "Starting event loop");
	m_isRunning = true;
	if (m_loop) {
...
#endif
	Log("SpectrumEventLoop", "Event loop stopped");
}

void SpectrumEventLoop::stop() {
	Log("SpectrumEventLoop", "Stopping event loop");
	if (!m_isRunning)
		return;
	if (m_loop) {
...
void SpectrumEventLoop::post(const Event& event) {
	...
	Log("SpectrumEventLoop", "Posting event " << ev);
