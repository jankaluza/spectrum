...
AdhocTag *AdhocAdmin::generateUserForm(const std::string &sessionId) {
	AdhocTag *adhocTag = new AdhocTag(sessionId, "transport_admin", "executing");
	adhocTag->setAction("next");
...
AdhocTag *AdhocAdmin::handleUserForm1(Tag *tag, const DataForm &form) {
...
AdhocTag *AdhocAdmin::handleUserForm2(Tag *tag, const DataForm &form) {
...
AdhocTag *AdhocAdmin::generateSendMessageForm(const std::string &sessionId) {
	AdhocTag *adhocTag = new AdhocTag(sessionId, "transport_admin", "executing");
	adhocTag->setAction("complete");
...
AdhocTag *AdhocAdmin::handleSendMessageForm(Tag *tag, const DataForm &form) {
...
AdhocTag *AdhocAdmin::generateRegisterUserForm(const std::string &sessionId) {
	AdhocTag *adhocTag = new AdhocTag(sessionId, "transport_admin", "executing");
	adhocTag->setAction("complete");
...
AdhocTag *AdhocAdmin::handleRegisterUserForm(Tag *tag, const DataForm &form) {
...
AdhocTag *AdhocAdmin::generateUnregisterUserForm(const std::string &sessionId) {
	AdhocTag *adhocTag = new AdhocTag(sessionId, "transport_admin", "executing");
	adhocTag->setAction("complete");
...
AdhocTag *AdhocAdmin::handleUnregisterUserForm(Tag *tag, const DataForm &form) {
	if (!form.hasField("user_jid"))
		return NULL;
...
AdhocTag *AdhocAdmin::generateListUsersForm(const std::string &sessionId) {
	AdhocTag *adhocTag = new AdhocTag(sessionId, "transport_admin", "executing");
...
static void userToCSV(gpointer key, gpointer v, gpointer data) {
...
AdhocTag *AdhocAdmin::handleExecutionInit(Tag *tag, const DataForm &form) {
	...
	if (result == "User") {
		...
		return generateUserForm(tag->findAttribute("sessionid"));
	}
	else if (result == "Send message to online users") {
		m_state = ADHOC_ADMIN_SEND_MESSAGE;
		return generateSendMessageForm(tag->findAttribute("sessionid"));
	}
	else if (result == "Register new user") {
		m_state = ADHOC_ADMIN_REGISTER_USER;
		return generateRegisterUserForm(tag->findAttribute("sessionid"));
	}
	else if (result == "Unregister user") {
		m_state = ADHOC_ADMIN_UNREGISTER_USER;
		return generateUnregisterUserForm(tag->findAttribute("sessionid"));
	}
	else if (result == "List online users") {
		m_state = ADHOC_ADMIN_LIST_USERS;
		return generateListUsersForm(tag->findAttribute("sessionid"));
	}
	return NULL;
}
...
AdhocTag *AdhocAdmin::handleAdhocTag(Tag *stanzaTag) {
	...
	if (m_state == ADHOC_ADMIN_INIT) {
		return handleExecutionInit(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_LIST_USERS) {
		return handleListUsersForm(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_USER) {
		return handleUserForm1(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_USER2) {
		return handleUserForm2(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_SEND_MESSAGE) {
		return handleSendMessageForm(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_REGISTER_USER) {
		return handleRegisterUserForm(tag, form);
	}
	else if (m_state == ADHOC_ADMIN_UNREGISTER_USER) {
		return handleUnregisterUserForm(tag, form);
	}
	return NULL;
}
