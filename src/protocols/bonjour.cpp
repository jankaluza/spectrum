...
std::list<std::string> BonjourProtocol::transportFeatures(){
	return m_transportFeatures;
}

std::list<std::string> BonjourProtocol::buddyFeatures(){
	return m_buddyFeatures;
}

std::string BonjourProtocol::text(const std::string &key) {
	if (key == "instructions")
		return _("Enter your Bonjour username:");
	else if (key == "username")
		return _("Bonjour username");
	return "not defined";
}

SPECTRUM_PROTOCOL(bonjour, BonjourProtocol)
