...
DictProtocol::DictProtocol(GlooxMessageHandler *main){
	m_main = main;
...
DictProtocol::~DictProtocol() {}

bool DictProtocol::isValidUsername(const std::string &str) {
	return true;
}

std::list<std::string> DictProtocol::transportFeatures(){
	return m_transportFeatures;
}

std::list<std::string> DictProtocol::buddyFeatures(){
	return m_buddyFeatures;
}

std::string DictProtocol::text(const std::string &key) {
	if (key == "instructions")
